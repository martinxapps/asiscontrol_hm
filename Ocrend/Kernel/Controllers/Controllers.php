...
abstract class Controllers
{
     ...
     *     'user_rol' => int, # Acceso para usuarios dependiendo del rol
     *
     */
    protected function __construct(IRouter $router, $configController = [])
    {
        ...
        $this->template->addGlobal('page_controller', $router->getController());
    ...
    private function knowVisitorPermissions()
    {
        ...
        # Sólamente usuarios permitidos
        if ($config['security']['access_control']) {
            # Sólamente usuarios permitidos
            if ($this->controllerConfig['users_logged'] && !in_array($this->user['rol_name'], $config['security']['modules'][$this->controller]['rol'])) {
                Helper\Functions::redir();
            }
        }
